...
/**
     ...
     * @return void
     */
    public function getPubDoc($libID = 'all', $module = 'all', $orderBy = 'id_desc', $pager = null)
    {
		//item doc 的 views 表示所有item数量
		$itemDocs = $this->dao->select('*,0 AS items')->from(TABLE_DOC)
            ->where('deleted')->eq(0)
			->andWhere('type')->eq('item')
			->orderBy($orderBy)->fetchAll('id');
		//$docItems = $this->dao->select('root,count(*) AS items')->from(TABLE_DOCITEM)
       //     ->where('root')->in(array_keys($itemDocs))
		//	->andWhere('flag')->eq(1)
		//	->groupBy('root')
		//	->fetchAll('root');
			
        $pubDocs = $this->dao->select('*,0 AS items')->from(TABLE_DOC)
            ->where('deleted')->eq(0)
            ->beginIF(is_numeric($libID))->andWhere('lib')->eq($libID)->fi()
            //->beginIF($module)->andWhere('module')->in($module)->fi()
			->orderBy($orderBy)->page($pager)->fetchAll('id');
		
		$pubDocs = $itemDocs+$pubDocs;
			
		//$visitDocs = $this->dao->select('doc,count(*) AS items')->from(TABLE_DOCVISIT)
        //    ->where('account')->eq($this->app->user->account)
		//	->andWhere('doc')->in(array_keys($pubDocs))
		//	->groupBy('doc')
		//	->fetchAll('doc');
		//$diffViews = $this->dao->select('doc,count(distinct(account)) AS diffView')->from(TABLE_DOCVISIT)
         //   ->where('doc')->in(array_keys($pubDocs))
		//	->groupBy('doc')
		//	->fetchAll('doc');
		foreach($pubDocs as $id=> $doc)
		{
			$doc->date			= date(DT_DATE5, strtotime($doc->addedDate));
			$doc->actionLabel	= '发布';
			$doc->visited		= 0;//array_key_exists($id,$visitDocs);
			$doc->visits		= 0;
			if($doc->type=='item')
			{
				$doc->objectLabel	= '规程';
				//$doc->items			= array_key_exists($id,$docItems)?$docItems[$id]->items:array();
				if($doc->visited) $doc->visits=$visitDocs[$id]->items;
			
			}
			else
				$doc->objectLabel	= '公告';
			$doc->objectLink	= helper::createLink('doc', 'view', "&docID=$doc->id");
			$doc->objectName	= $doc->title;
			
			//if(array_key_exists($id,$diffViews))
			//	$doc->diffViews	= $diffViews[$id]->diffView;
			//else
				$doc->diffViews	= 0;
		}
		return $pubDocs;
